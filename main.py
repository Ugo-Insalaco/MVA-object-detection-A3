...
def opts() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        default="vanilla",
        metavar="D",
        help="name of the selected dataset. vanilla/ flip_augment",
    )
    parser.add_argument(
        "--model_name",
        type=str,
...
def main():
    ...
    val_loader = torch.utils.data.DataLoader(
        datasets.ImageFolder(os.path.join(args.data, "val_images"), transform=data_transforms),
    ...
    train_loader = torch.utils.data.DataLoader(
        get_dataset(args.dataset, os.path.join(args.data,"train_images"), transform=data_transforms), 
        batch_size=args.batch_size,
        shuffle=True,
        num_workers=args.num_workers,
    )
    ...
    if not args.optimizer in supported_optimizers:
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        model_file = os.path.join(args.experiment, f"model_{str(epoch)}.pth")
